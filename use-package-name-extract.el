...
(defun upe-walk (tree)
  (cond ((atom tree)                      nil)
        ((and (eq (nth 0 tree) 'when)
              (eq (nth 1 tree) 'nil))     nil) ; ignore parts commented out with (when nil ...)
        ((and (eq (nth 0 tree) 'unless)
              (eq (nth 1 tree) 't))       nil) ; ignore parts commented out with (unless t ...)
        ((eq (nth 0 tree) 'if)    ; handling special cases of (if nil ...) and (if t ...)
         (cond ((eq (nth 1 tree) 'nil) (upe-walk (nthcdr 3 tree)))
               ((eq (nth 1 tree) 't)   (upe-walk (nth 2 tree)))
               (t                      (append (upe-walk (nth 2 tree))          ; non-"constant" condition -> walk both arms
                                               (upe-walk (nthcdr 3 tree))))))
